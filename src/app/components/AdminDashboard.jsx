...
import { Shield, CheckCircle, XCircle, Users, Package, IndianRupee, User, LogOut, Ban, Clock, TrendingUp } from 'lucide-react';
...
import { getAllTickets, getPendingTickets, verifyTicket } from '../../services/ticketService';
...
export default function AdminDashboard() {
    ...
    const loadData = async () => {
        ...
        setPendingTickets(Array.isArray(pending) ? pending : []);
    ...
    const confirmVerify = async (approve) => {
        if (!selectedTicket)
            return;
        const result = await verifyTicket(selectedTicket.id, approve);
        if (result.success) {
            toast.success(approve ? 'Ticket approved successfully' : 'Ticket rejected');
        }
        else {
            toast.error(result.message || 'Unable to update ticket status');
        }
    ...
    const handleToggleBlockUser = async (userId, userName, isBlocked) => {
        ...
        if (confirm(`Are you sure you want to ${action} ${userName}?`)) {
            ...
            if (result.success) {
                ...
                toast.error(result.message || `Unable to ${action} user`);
            }
        }
    };
        ...
        <Tabs defaultValue="pending" className="space-y-6">
          <TabsList>
            <TabsTrigger value="pending" className="gap-2">
              ...
              {pendingTickets.length > 0 && (<Badge variant="secondary" className="ml-1 bg-yellow-100 text-yellow-700">
          ...
          <TabsContent value="pending" className="space-y-6">
            <Card className="border-2">
              ...
              <div className="p-6">
                {pendingTickets.length === 0 ? (<div className="text-center py-12">
                    <CheckCircle className="size-16 text-gray-300 mx-auto mb-4"/>
                    <p className="text-gray-600">No pending tickets</p>
                  </div>) : (<div className="space-y-4">
                    {pendingTickets.map((ticket) => (<Card key={ticket.id} className="p-6 border-2">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="ml-6 text-right">
                            ...
                            <div className="space-y-2">
                              <Button onClick={() => handleVerifyTicket(ticket)} className="w-full bg-green-600 hover:bg-green-700" size="sm">
                                <CheckCircle className="mr-2 size-4"/>
                                Verify
                              </Button>
                              <Button onClick={() => confirmVerify(false)} variant="outline" className="w-full text-red-600 border-red-300 hover:bg-red-50" size="sm">
            ...
            <Card className="border-2">
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {allTickets.map((ticket) => (<TableRow key={ticket.id}>
                        ...
                        <TableCell>
                          <Badge variant={ticket.status === 'BOOKED' ? 'default' :
                ticket.status === 'AVAILABLE' || ticket.status === 'VERIFIED' ? 'secondary' :
                    'outline'} className={ticket.status === 'PENDING' ? 'bg-yellow-100 text-yellow-700' :
            ...
            <Card className="border-2">
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {allUsers.map((u) => (<TableRow key={u.id} className={u.isBlocked ? 'bg-red-50' : ''}>
                        <TableCell>
                        ...
                        <TableCell>
                          <Badge variant="secondary" className={u.role === 'admin' ? 'bg-purple-100 text-purple-700' :
                u.role === 'seller' ? 'bg-blue-100 text-blue-700' :
                    'bg-green-100 text-green-700'}>
                        ...
                        <TableCell>
                          {u.role !== 'admin' && (<Button variant="outline" size="sm" onClick={() => handleToggleBlockUser(u.id, u.name, u.isBlocked)} className={u.isBlocked ? 'text-green-600' : 'text-red-600'}>
